 ...
 * 자동 매매 전략 서비스
 * 
 * <p>정량적 규칙 엔진과 거시경제 전략 엔진을 결합하여 자동 매매 결정을 수행합니다.
 * 기술적 분석 결과와 거시경제 지표를 종합하여 매수/매도/보유를 결정하며,
 * 결정된 경우 주문 요청 DTO를 생성합니다.</p>
 * 
 * <p>AI 예측 서비스와 연동하여 더 정확한 매매 결정을 내릴 수 있도록 설계되었습니다.</p>
 * 
...
public class TradingStrategyService {
     ...
     * 자동 매매 결정
     * 
     * <p>기술적 분석과 거시경제 지표를 종합하여 자동 매매 결정을 수행합니다.
     * 거래 설정에서 자동 매매가 비활성화된 경우 null을 반환합니다.</p>
     * 
     * <p>결정 프로세스:
     * <ol>
    ...
    public OrderRequestDto decideTradingAction(String accountNo, String symbol) {
        ...
        MacroEconomicStrategyEngine.InvestmentStrategy macroStrategy = 
                macroEconomicStrategyEngine.decideStrategy(indicators);
        
        // 4. 정량적 규칙 엔진을 통한 최종 매매 결정
        QuantitativeRuleEngine.TradingDecision decision = 
                quantitativeRuleEngine.decide(technicalAnalysis, macroStrategy);
     ...
     * 매수 주문 생성
     * 
     * <p>매수 결정에 따라 주문 요청 DTO를 생성합니다.
     * 거래 설정의 최대 투자금액과 포지션 크기 비율을 기반으로 매수 수량을 계산합니다.</p>
     * 
     * @param accountNo 계좌번호
     * @param symbol 종목코드
     * @param setting 거래 설정
     ...
     * 매도 주문 생성
     * 
     * <p>매도 결정에 따라 주문 요청 DTO를 생성합니다.
     * 현재는 보유 종목 정보가 없어 구현이 보류되어 있습니다.</p>
     * 
     * <p>TODO: 보유 종목 정보를 조회하여 매도 수량을 결정하는 로직 구현 필요</p>
     * 
     * @param accountNo 계좌번호
     * @param symbol 종목코드
     * @param setting 거래 설정
     ...
     * 여러 종목에 대한 자동 매매 결정
     * 
     * <p>여러 종목에 대해 순차적으로 자동 매매 결정을 수행합니다.
     * 각 종목에 대해 주문 요청이 생성된 경우에만 결과에 포함됩니다.</p>
     * 
     * @param accountNo 계좌번호
