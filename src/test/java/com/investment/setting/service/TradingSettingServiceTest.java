...
class TradingSettingServiceTest {
    ...
    void 설정_저장_실패_최대금액_미만() {
        // given
        settingDto.setMaxInvestmentAmount(new BigDecimal("5000"));
        settingDto.setMinInvestmentAmount(new BigDecimal("10000"));
        
        // when & then
        DomainException exception = assertThrows(DomainException.class, 
                () -> tradingSettingService.saveSetting("1234567890", settingDto));
    ...
    void 설정_저장_실패_리스크레벨_범위초과() {
        // given
        settingDto.setRiskLevel(new BigDecimal("1.5")); // 1.0 초과
