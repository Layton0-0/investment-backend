...
- [x] **퀀트 시스템 설계서 대조 점검 반영 (시장 급락 게이트·전략 비교)**
  설계서 "돈만 넣으면 규칙대로 자동 운용되는 퀀트 시스템" 대비 갭 구현 및 문서화. **시장 급락 시 동결**: MarketCrashGateService 도입 — 벤치마크(기본 SPY) 전일 일일 낙폭이 설정값(기본 5%) 이상이면 당일 신규 매수 중단. RiskProperties(market-crash-gate-enabled, market-crash-daily-drop-pct, benchmark 심볼/시장), PipelineExecutionScheduler 연동. 00-strategy-registry.md §2.9, decisions.md ADR 28. **전략 비교 화면**: GET /api/v1/strategies/comparison(market 선택), StrategyComparisonService(GovernanceCheckResult 최신 per market+strategyType), StrategyComparisonItemDto(MDD·Sharpe·설명). 프론트 /strategies/:market 에 전략 비교 테이블(CAGR·MDD·Sharpe·최근 실행) 추가. 02-api-endpoints.md §4.0, strategyApi.getStrategyComparison.
- [x] **시스템 설정 DB화 및 Ops 시스템 설정 화면**
  서버 전역 기본값(예: pipeline.auto-execute, allow-real-execution)을 DB(TB_SYSTEM_SETTINGS)에 저장하고, DB 값 우선·application.yml fallback. SystemSetting 엔티티·Repository·SystemSettingService(whitelist, getBoolean/getBigDecimal, 5분 캐시)·SystemSettingController GET/PUT /api/v1/system/settings(ADMIN 전용). PipelineExecutor·PipelineExecutionScheduler·PipelineExitScheduler·IntradayBreakoutScheduler에서 서버 기본값을 SystemSettingService에서 조회하도록 변경. Ops 메뉴 "시스템 설정"(/ops/settings) 추가, 목록·Boolean/숫자 편집·저장 연동. [02-api-endpoints.md §5.4](../04-api/02-api-endpoints.md), decisions.md ADR 반영.
- [x] **로컬 Docker 자동투자 로그 검증 및 개선 설계 구현**
  로그 가시성: 파이프라인/로보 스킵 메시지·KRX/US 수집 스킵을 INFO/WARN으로 상향(PipelineExecutionScheduler, RoboRebalanceScheduler, KrxApiClient, KrxCollectionService). QA 문서(plans/qa)에 로그 수집 명령·시간대별 체크리스트·원인→대응 매트릭스·배치_실행_이력_점검.sql 추가. 데이터 수집: US HTTP 재시도(지수 백오프, retry-max/retry-initial-ms), 실패·0건 시 Discord 알림 옵션(failure-alert-enabled). 배치 Job 실패 시 Discord 알림(investment.batch.failure-alert-enabled). 설정 검증: DataCollectionHealthIndicator(actuator/health에 usCollectorConfigured·krxAuthConfigured). 01-local-setup-complete.md에 US/KRX 수집 0건 시 env·헬스 확인 절차 추가. 13-manual-operator-tasks.md에 Backend 재시작이 16:00/17:00 이후인 경우 수동 트리거(krx-daily, us-daily) 절차 추가.
