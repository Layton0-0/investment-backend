...
| **현재 전략 문서 버전** | v1.0 |
| **최종 갱신일** | 2026-01-31 |
| **코드 참조** | `factor.service.*`, `factor.execution.ExitRuleService`, `application.yml` (investment.factor, investment.pipeline) |
...
- **설정**: `investment.factor.kelly-p` (기본 0.6), `investment.factor.kelly-b` (기본 2.0)
...
| **한국(KR)** | 수급 강도 (Smart Money Intensity) | 최근 5일 누적 순매수 금액이 시총의 일정 비율(예: 0.5%) 초과 시 매수 시그널 | TB_ORDER_FLOW 기반. `smart-money-intensity-threshold-pct`. 데이터 없으면 0. |
...
| **중기** | Time-Cut | 매수 후 N일 내 목표 수익률 미도달 시 전량 매도 | `StrategyPosition.timeCutDays`, `targetReturnPct`, `ExitRuleService` |
...
| Time-Cut | N일 내 목표 수익률 미도달 시 매도 | (엔티티: timeCutDays, targetReturnPct) | ExitRuleService |
