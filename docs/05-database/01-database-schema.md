...
1. **TB_ORDERS**: 주문 정보
2. **TB_TRADING_SETTINGS**: 거래 설정
3. **TB_PORTFOLIOS**: 보유 종목
4. **TB_STRATEGIES**: 투자 전략
5. **TB_TRADING_PORTFOLIOS**: 트레이딩 포트폴리오
6. **TB_TRADING_PORTFOLIO_ITEMS**: 트레이딩 포트폴리오 종목
...
| ACCOUNT_NO | VARCHAR(20) | NOT NULL, UNIQUE | 계좌번호 |
...
### 3.5 TB_TRADING_PORTFOLIOS (트레이딩 포트폴리오)
...
### 3.6 TB_TRADING_PORTFOLIO_ITEMS (트레이딩 포트폴리오 종목)
...
### 7.1 UNIQUE 제약
- `TB_TRADING_SETTINGS.ACCOUNT_NO`: 계좌당 설정은 1개
- `TB_PORTFOLIOS.ACCOUNT_NO + SYMBOL`: 계좌당 종목은 1개
- `TB_STRATEGIES.ACCOUNT_NO + STRATEGY_TYPE`: 계좌당 전략 타입은 1개
